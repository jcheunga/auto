...
export interface AppRuntimeStats {
  running: boolean;
  activeWorkers: number;
  lockedItemIds: string[];
}

export interface AppWebhookResult {
  challenge?: string;
...
export interface AppOrchestrator {
  getRuntimeStats(): AppRuntimeStats;
  handleMondayWebhook(payload: unknown): Promise<AppWebhookResult>;
}

export interface AppWorkflowAgent {
  cleanupWorktrees(input: { owner?: string; repo?: string; force?: boolean }): Promise<Record<string, unknown>>;
}
...
export interface AppDependencies {
  ...
  workflowAgent: AppWorkflowAgent;
  config: AppConfig;
  publicDir?: string;
}
...
export function createApp({ db, orchestrator, workflowAgent, config, publicDir = path.resolve(process.cwd(), "public") }: AppDependencies): Express {
  const app = express();
